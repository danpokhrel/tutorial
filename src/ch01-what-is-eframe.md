...
4. **Missing wgpu backends.** eframe enables *no* wgpu backends by default. On Windows you need `dx12`, on macOS `metal`, on the web `webgl`/`webgpu`. Forgetting this gives you a blank window or a runtime panic. We set this up in [Chapter 2](./ch02-project-setup.md).
